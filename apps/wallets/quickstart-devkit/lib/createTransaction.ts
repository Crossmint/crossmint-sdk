...
export async function createTokenTransferTransaction(from: string, to: string, tokenMint: string, amount: number) {
    ...
    const senderTokenAccount = await getAssociatedTokenAddress(tokenMintPublicKey, fromPublicKey, allowOffCurve);
    const recipientTokenAccount = await getAssociatedTokenAddress(tokenMintPublicKey, toPublicKey, allowOffCurve);
