...
import { SIGNER_TYPES, type SignerType } from "../types/signer";
import { getSignerLocator, signerLabel, buildSignerConfig } from "../utils/signerUtils";
...
interface PermissionsProps {
    wallet: any;
    createDeviceSigner?: (...args: any[]) => any;
    createPasskeySigner?: (...args: any[]) => any;
...
export function Permissions({
    ...
    const handleUseSigner = async () => {
        ...
        try {
            ...
            if (selectedLocator.startsWith("external-wallet:") && buildExternalWalletSignerFn && usePrivateKey) {
                const config = buildExternalWalletSignerFn(wallet.chain, usePrivateKey);
                ...
                await wallet.useSigner(config as any);
                setStatus(`Active signer: ${selectedSigner.type} (${selectedLocator})`);
            }
        } catch (e: any) {
    ...
    return (
        <View style={{ backgroundColor: "#F7F8FA", borderRadius: 12, padding: 16 }}>
            ...
            {isExternalWalletSelected && buildExternalWalletSignerFn && (
                <TextInput
                    style={inputStyle}
                    secureTextEntry
            ...
            {signerType === "external-wallet" &&
                (buildExternalWalletSignerFn ? (
                    <TextInput
                        ...
                        onChangeText={(v) => setField("privateKey", v)}
                        autoCapitalize="none"
                    />
                ) : (
                    <TextInput
