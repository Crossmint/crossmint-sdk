...
import { AUTH_ROOT_ENDPOINT, AUTH_SDK_ROOT_ENDPOINT, SDK_NAME, SDK_VERSION } from "./utils/constants";
...
export class CrossmintAuth {
    ...
    protected async refresh(refreshToken: string): Promise<AuthMaterialWithUser> {
        const result = await this.apiClient.post(`${AUTH_ROOT_ENDPOINT}/refresh`, {
        ...
        return {
            jwt: resultJson.jwt,
            refreshToken: {
                secret: resultJson.refreshToken.token,
                expiresAt: resultJson.refreshToken.expiresAt,
            },
