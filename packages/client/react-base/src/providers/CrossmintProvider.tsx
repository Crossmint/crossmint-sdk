...
export function CrossmintProvider({
  ...
  if (!crossmintRef.current) {
    crossmintRef.current = new Proxy<Crossmint>(
      createCrossmint({ apiKey, overrideBaseUrl, appId, extensionId }),
      {
        set(target, prop, value) {
          ...
          if (
            prop === "experimental_customAuth" &&
            target.experimental_customAuth !== value
          ) {
            setVersion((v) => v + 1);
          }
  ...
  const experimental_setCustomAuth = useCallback(
    (customAuthParams?: CustomAuth) => {
      if (!crossmintRef.current) {
        throw new Error("CrossmintProvider is not initialized");
      }
      ...
      if (
        !isEqual(customAuthParams, crossmintRef.current.experimental_customAuth)
      ) {
