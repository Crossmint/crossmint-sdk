...
> extends LoggerWrapper {
    ...
    async signMessage(message: string | Uint8Array) {
        return logPerformance(
            ...
            { chain: this.chain, address: this.getAddress() }
        );
    }
        ...
        return logPerformance(
            ...
            async () => {
                try {
                    ...
                    const txResult = await this.kernelClient.sendTransaction({
                        to: to as `0x${string}`,
                        ...
                        case "sft": {
                            ...
                            const { request } = await publicClient.simulateContract({
                                ...
                                args: [
                                    ...
                                    toAddress,
                                    tokenId,
                                    (config as SFTTransferType).quantity,
                                    "0x00",
                                ],
                                ...this.getLegacyTransactionFeesParamsIfApply(),
                            });
        ...
        return logPerformance(
            ...
            async () => {
                try {
                    ...
                    throw new Error(
                        `Error setting custodian for tokens. If this error persists, please contact support.`
                    );
                }
            },
            { chain: this.chain, address: this.getAddress() }
    ...
    async upgradeVersion() {
        return logPerformance(
            ...
            async () => {
                try {
                    const sessionKeys = await this.crossmintService!.createSessionKey(
