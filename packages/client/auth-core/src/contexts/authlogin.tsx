...
const getJwtFromCookie = (): string | null => {
	...
	const crossmintSession = document.cookie
		.split("; ")
		.find((row) => row.startsWith("crossmint-session"));
...
export function AuthProvider({ children, apiKey }: AuthProviderParams) {
	...
	const crossmintService = useMemo(
		() => CrossmintServiceFactory.create(apiKey, jwtToken),
		[apiKey, jwtToken],
	);
	...
	useEffect(() => {
		const crossmintSession = document.cookie
			.split("; ")
			.find((row) => row.startsWith("crossmint-session"));
		const crossmintSessionCookie = crossmintSession
			? crossmintSession.split("=")[1]
			: null;
	...
	const logout = () => {
		document.cookie =
			"crossmint-session=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
		setJwtToken(null);
	};
