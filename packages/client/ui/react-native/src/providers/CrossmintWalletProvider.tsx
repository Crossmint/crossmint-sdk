...
const throwNotAvailable = (functionName: string) => () => {
  throw new Error(
    `${functionName} is not available. Make sure you're using an email signer wallet.`
  );
};
...
export const CrossmintWalletEmailSignerContext =
  createContext<CrossmintWalletEmailSignerContext>({
    needsAuth: false,
...
export function CrossmintWalletProvider({
  ...
  const sendEmailWithOtpRef = useRef<() => Promise<void>>(
    throwNotAvailable("sendEmailWithOtp")
  );
  const verifyOtpRef = useRef<(otp: string) => Promise<void>>(
