...
import {
	...
	getJWTExpiration,
} from "@crossmint/client-sdk-auth-core/client";
import {
	type EVMSmartWallet,
	SmartWalletSDK,
...
vi.mock("@crossmint/client-sdk-auth-core/client", async () => {
	const actual = await vi.importActual(
		"@crossmint/client-sdk-auth-core/client",
	);
	return {
		...actual,
	...
	return (
		<div>
			...
			<button
				...
				onClick={() => setRefreshToken("mock-refresh-token")}
			>
				Set Refresh Token
			</button>
			<button
				data-testid="clear-refresh-token-button"
				onClick={() => setRefreshToken(undefined)}
			>
				Clear Refresh Token
			</button>
		</div>
...
describe("CrossmintAuthProvider", () => {
	...
	beforeEach(() => {
		...
		vi.mocked(CrossmintAuthService).mockImplementation(
			() => mockCrossmintAuthService as any,
		);
	});
	...
	test("Happy path", async () => {
		...
		await waitForSettledState(() => {
			...
			expect(getByTestId("auth-status").textContent).toBe("logged-in");
			expect(getByTestId("refresh-token").textContent).toBe(
