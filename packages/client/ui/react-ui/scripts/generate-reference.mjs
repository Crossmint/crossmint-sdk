...
function buildProviders(product) {
    ...
    emit(`description: React context providers for ${product.description}`);
    emit("---");
    emit("");
...
function buildHooks(product) {
    ...
    emit("---");
    emit(`title: Hooks`);
    emit(`description: React hooks for ${product.description}`);
    emit("---");
    emit("");
    ...
    for (const name of hooks) {
        ...
        let members = extractReturnMembers(node);
        if (!members.length && MANUAL_RETURNS[name]) {
            members = MANUAL_RETURNS[name];
        }
...
function buildComponents(product) {
    ...
    emit(`description: React components for ${product.description}`);
    emit("---");
    emit("");
