...
describe("Wallet - signers()", () => {
    ...
    describe("error cases", () => {
        ...
        it("should fall back to config signer data when getSigner returns an error", async () => {
            const mockWalletResponse: GetWalletSuccessResponse = {
                chainType: "evm",
                type: "smart",
                address: wallet.address,
            ...
            // First signer (device from wallet creation) returns an error from getSigner
            mockApiClient.getSigner.mockResolvedValueOnce({ error: "signer not found" } as any).mockResolvedValueOnce({
                type: "external-wallet",
                address: "0xsigner2",
                locator: "external-wallet:0xsigner2",
                chains: {
            ...
            // Both signers should be returned: first falls back to config data, second from API
            expect(signers).toHaveLength(2);
            expect(signers[0].type).toBe("device");
            expect(signers[0].locator).toBe("device:abc123");
            expect(signers[0].status).toBe("success");
            expect(signers[1].type).toBe("external-wallet");
            expect(signers[1].locator).toBe("external-wallet:0xsigner2");
        });

        it("should fall back to config signer data when getSigner throws", async () => {
            const mockWalletResponse: GetWalletSuccessResponse = {
                chainType: "evm",
                type: "smart",
                address: wallet.address,
            ...
            mockApiClient.getWallet.mockResolvedValue(mockWalletResponse);
            mockApiClient.getSigner.mockRejectedValueOnce(new Error("Network error"));
        ...
        it("should include wallet-creation signers without chains field alongside chain-approved signers", async () => {
            const mockWalletResponse: GetWalletSuccessResponse = {
                chainType: "evm",
                type: "smart",
                address: wallet.address,
                    ...
                    delegatedSigners: [
                        {
                            type: "device",
                            publicKey: { x: "1", y: "2" },
                            locator: "device:wallet-creation-device",
                        },
                        {
                            type: "device",
                            publicKey: { x: "3", y: "4" },
                            locator: "device:later-added-device",
                        },
                        {
                            type: "email",
            ...
            // First device signer: no chains field (wallet-creation signer)
            // Second device signer: has chains field with current chain
            // Email signer: has chains field with current chain
            mockApiClient.getSigner
                .mockResolvedValueOnce({
                    type: "device",
                    publicKey: { x: "1", y: "2" },
            ...
            expect(signers[0].locator).toBe("device:wallet-creation-device");
            expect(signers[0].status).toBe("success");
            expect(signers[1].locator).toBe("device:later-added-device");
            expect(signers[1].status).toBe("success");
            expect(signers[2].locator).toBe("email:user@example.com");
            expect(signers[2].status).toBe("success");
        });
    });
});
