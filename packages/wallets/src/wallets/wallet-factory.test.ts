    ...
    describe("Error Cases", () => {
        it("should throw error when delegated signers are provided but wallet has none", async () => {
            ...
            await expect(walletFactory.getOrCreateWallet(mockValidSolanaArgs)).rejects.toThrow(
                new WalletCreationError(
                    `2 delegated signer(s) specified, but wallet "9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM" has no delegated signers. When 'delegatedSigners' is provided to a method that fetches an existing wallet, that wallet must have matching delegated signers.`
                )
            );
        });
        ...
        it("should throw error when number of delegated signers does not match", async () => {
            ...
            await expect(walletFactory.getOrCreateWallet(argsWithFewerSigners)).rejects.toThrow(
                new WalletCreationError(
                    `1 delegated signer(s) specified, but wallet "9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM" has 2 delegated signers. When 'delegatedSigners' is provided to a method that fetches an existing wallet, that wallet must have matching delegated signers.`
                )
            );
        });
        ...
        it("should throw error when number of delegated signers does not match (more provided)", async () => {
            ...
            await expect(walletFactory.getOrCreateWallet(argsWithMoreSigners)).rejects.toThrow(
                new WalletCreationError(
                    `3 delegated signer(s) specified, but wallet "9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM" has 2 delegated signers. When 'delegatedSigners' is provided to a method that fetches an existing wallet, that wallet must have matching delegated signers.`
                )
            );
        });
    ...
    describe("Edge Cases", () => {
        ...
        it("should throw error when args has empty array but wallet has signers", async () => {
            ...
            await expect(walletFactory.getOrCreateWallet(argsWithEmptyDelegatedSigners)).rejects.toThrow(
                new WalletCreationError(
                    `0 delegated signer(s) specified, but wallet "9WzDXwBbmkg8ZTbNMqUxvQRAyrZzDsGYdLVL9zYtAWWM" has 2 delegated signers. When 'delegatedSigners' is provided to a method that fetches an existing wallet, that wallet must have matching delegated signers.`
                )
            );
        });
