    ...
    describe("createWallet with device signer", () => {
        it("should not inject device signer for Solana wallets", async () => {
            ...
            const mockDeviceSignerKeyStorage = {
                ...
                generateKey: vi.fn().mockResolvedValue("dummyKey"),
            ...
            // Verify that no device signer was added for Solana
            const call = mockApiClient.createWallet.mock.calls[0]?.[0];
            const delegatedSigners = call?.config?.delegatedSigners ?? [];
            const hasDeviceSigner = delegatedSigners.some(
                (s: any) => s?.signer?.type === "device" || s?.type === "device"
            );
            expect(hasDeviceSigner).toBe(false);
        });
