...
export class SolanaDelegatedSignerService {
    ...
    public async registerDelegatedSigner(signer: string, adminSigner?: SolanaNonCustodialSigner) {
        const response = (await this.apiClient.registerSigner(this.walletLocator, { signer })) as Extract<
            Awaited<ReturnType<typeof this.apiClient.registerSigner>>,
            { transaction: { id: string } }
        >;

        const transactionId = response.transaction.id;
        await this.transactionsService.approveTransaction(transactionId, adminSigner ? [adminSigner] : []);
