...
type PendingApproval = NonNullable<
    NonNullable<CreateTransactionResponse["approvals"]>["pending"]
>[number];
            ...
            pendingApprovals.map(async (approval) => {
                ...
                return {
                    signature: bs58.encode(
                        await signer.signMessage(bs58.decode(approval.message))
                    ),
                    signer: approval.signer,
                };
            })
        );
        const transaction = await this.apiClient.approveTransaction(
            this.walletLocator,
            transactionId,
            {
                approvals,
